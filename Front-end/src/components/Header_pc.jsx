   ...
   <div className="my-4 px-3 flex justify-between items-center z-[9999] ">
    <ul className="flex gap-4 items-center font-semibold text-[15px] xl:text-[17px]">
     <li className=" cursor-pointer relative group/group1">
      دسته بندی کالا ها
      <ul className="hidden absolute rigth-0 top-5 px-2 py-2 group-hover/group1:flex flex-col gap-5 bg-white rounded-xl shadow-md shadow-black">
      ...
      to={`/category/${item.href}`}
          className={` px-4 w-[120px] py-2 text-center  text-nowrap p-1d rounded-r-lg hover:bg-sky-100 ${menu.title == item.title ? " bg-sky-100" : ""} `}
         >
          {item.title}
         </NavLink>
        </ul>
        ...
        <ul
         className={` grid gap-1 grid-cols-3 absolute top-0 ${
          menu.title == "موبایل"
           ...
           ? "-top-[193px]"
           : "-top-[254px]"
         } p-1 right-[120px] w-[600px] lg:w-[700px] h-[290px] bg-sky-100 rounded-xl  `}
        >
         { menu.sub.map((ite, k) => (
